defmodule Braise.EmberModelTemplate do
  ...
  defp custom_actions(model) do
    Enum.map(model.actions.non_restful, fn(link_action)-> non_restful_javascript(model.name, link_action) end)
    |> Enum.join(",\n")
  end
  ...
  defp non_restful_javascript(model_name, link_action) do
    action_name = link_action.name
    """
      #{action_name}: function() {
        var _this = this;
